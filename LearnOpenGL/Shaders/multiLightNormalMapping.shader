...
void main()
{
    mat3 NormalMatrixMat3 = mat3(NormalMatrix[gl_InstanceID]); // moet/kan originele NormalMatrix geen mat3 zijn?
    vec3 T = normalize(NormalMatrixMat3 * aTangent);
    vec3 B = normalize(NormalMatrixMat3 * aBitangent); // there is no need for the bitangent variable in the vertex shader, we can calculate the bitangent ourselves, wat is beter?
    vec3 N = normalize(NormalMatrixMat3 * aNormal);
    //    T = normalize(T - dot(T, N) * N);
    //    vec3 B = cross(N, T);
    mat3 TBN = transpose(mat3(T, B, N)); // transforms any vector from view space to tangent space

    //vec3 T = normalize(modelViewMatrix[gl_InstanceID] * vec4(aTangent, 0.0f)).xyz;
    //vec3 B = normalize(modelViewMatrix[gl_InstanceID] * vec4(aBitangent, 0.0f)).xyz; // there is no need for the bitangent variable in the vertex shader, we can calculate the bitangent ourselves, wat is beter?
    //vec3 N = normalize(modelViewMatrix[gl_InstanceID] * vec4(aNormal, 0.0f)).xyz;
    //mat3 TBN = transpose(mat3(T, B, N)); // transforms any vector from view space to tangent space

    // is die normalmatrix wel geschikt met view er al in? want de translation is er uit...
...
//vec3 normalView = normalize((texture(material.normal1, vs_out.TexCoords).rgb) * 2.0 - 1.0); // normal in tangent space

vec3 viewDirView = normalize(-vs_out.FragPosView);
//vec3 viewDirView = vs_out.TBN * normalize(-vs_out.FragPosView);
...
vec3 CalcDirLight(DirLight light)
{
    ...
    vec3 specular = light.color * spec * textureSpecular * 1.0f;
